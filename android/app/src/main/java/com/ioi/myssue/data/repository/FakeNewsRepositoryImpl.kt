...
import com.ioi.myssue.data.dto.response.NewsContentDto
...
class FakeNewsRepositoryImpl @Inject constructor(
    ...
    override suspend fun getHotNews(): List<News> =
        List(5) { idx ->
            News(
                title = "갈 곳도, 볼 것도, 사랑도 없다”…절벽으로 내몰리는 노인들",
                content = blocks,
                url = "https://example.com/$idx",
                img = blocks.firstImageUrl() ?: "",
                category = "사회",
            ...
            val raw = n.createdAt
            n.copy(
                createdAt = time.toDisplay(raw),
                relativeTime = time.toRelative(raw)
            )
        }
