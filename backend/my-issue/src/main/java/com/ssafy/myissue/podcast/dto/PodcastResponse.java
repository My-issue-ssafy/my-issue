...
public record PodcastResponse(Long podcastId, String category, String thumbnail, String podcastUrl, List<String> keyword, List<Subtitles> subtitles) {
    public static PodcastResponse of(Podcast podcast, PodcastNews podcastNews, List<Subtitles> subtitles) {
        try {
            ...
            return new PodcastResponse(podcast.getId(), podcastNews.getNews().getCategory(), podcastNews.getNews().getThumbnail(), podcast.getAudio(), keywords, subtitles);
        } catch (Exception e) {
